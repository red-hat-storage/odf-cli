...
func newChecker(dynClient *dynamicfake.FakeDynamicClient, k8sClient *fake.Clientset) *NooBaaHealthChecker {
	return &NooBaaHealthChecker{
...
func TestNooBaaHealthChecker(t *testing.T) {
	...
	t.Run("no NooBaa CR present", func(t *testing.T) {
		...
		checker := newChecker(dynClient, k8sClient)
	...
	t.Run("phase Ready with running pod", func(t *testing.T) {
		...
		assert.Equal(t, health.CategoryObjectStorage, result.Category)
...
func TestNooBaaHealthCheckerName(t *testing.T) {
	checker := &NooBaaHealthChecker{}
	assert.Equal(t, "NooBaa Health", checker.Name())
}
