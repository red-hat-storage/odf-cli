...
const checkNooBaaHealth = "NooBaa Health"
...
// NooBaaHealthChecker implements health.HealthChecker to check NooBaa CR phase
// and core pod status.
type NooBaaHealthChecker struct {
	Ctx              context.Context
...
func (n *NooBaaHealthChecker) Name() string {
	return checkNooBaaHealth
}

func (n *NooBaaHealthChecker) Check() health.CheckResult {
	phase, creationTime, found, err := getNooBaaPhase(n.Ctx, n.DynamicClient, n.ClusterNamespace)
	if !found {
	...
	result := health.CheckResult{
		...
		Category: health.CategoryObjectStorage,
	}
	...
	pods, err := n.K8sClient.CoreV1().Pods(n.ClusterNamespace).List(n.Ctx, metav1.ListOptions{
	...
	if err != nil {
		...
		result.Status = worseStatus(result.Status, health.StatusWarning)
		return result
	}
	...
	for i := range pods.Items {
		...
		if containerName, crashing := isContainerCrashLooping(pod); crashing {
			...
			result.Status = worseStatus(result.Status, health.StatusCritical)
...
func worseStatus(a, b health.CheckStatus) health.CheckStatus {
	if a > b {
		return a
	}
	return b
